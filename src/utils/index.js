...
export const formatTime=(time)=>{//格式化时间戳
    let date=new Date(time)
    let y=date.getFullYear(),m=(date.getMonth()+1) < 10 ? '0'+(date.getMonth()+1) : date.getMonth()+1,d=date.getDate()<10 ? '0'+date.getDate() : date.getDate();
    return `${y}-${m}-${d}`
}
//深拷贝
